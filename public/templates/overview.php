...
if (isset($_POST)) {
    foreach ($_POST as $key => $value) {
        if ($value == "Rehash")
            Dalek::do_rehash($key);
        if ($value == "Remove") {
            $tok = explode(MAGIC_SEP, base64_decode($key));
            Dalek::tkl_remove($tok[0], $tok[1]);
        }
        if ($value == "Oper") {
            $nick = base64_decode($key);
            Dalek::send_oper($nick);
        }
        if ($value == "Deoper") {
            $nick = base64_decode($key);
            Dalek::del_oper($nick);
...
Dalek_UserList::$chancount = count(Dalek_ChanList::$list);
        ...
        ?>
                <tr>
                    ...
                    elseif (strpos($user->usermodes, "o") != false)
                        echo "<td>" . dalek_get_oper_type($user->UID) . "<form method=\"post\"><input type=\"submit\" onclick=\"return  confirm('Are you sure you want to remove $user->nick\'s oper?')\" name=\"" . base64_encode($user->nick) . "\" value=\"Deoper\"></input></form></td>";
                    elseif (!strpos($user->usermodes, "o")) {
                        $tracker = 0;
                        $user_obj = get_user_by("slug", strtolower($user->account));

                        if ($user_obj) {
                            $class = NULL;
                            $user_meta = get_userdata($user_obj->id);
                            foreach ($user_meta->roles as $cap) {
                                if (strstr($cap, "irc_")) {
                                    $class = $cap;
                                    break;
                                }
                                if (strstr($cap, "administrator")) {
                                    $class = $cap;
                                }
                            }
                            if ($class) {
                    ...
                    ?>
                                <td>
                                    <form method="post">
                                        <input type="submit" onclick="return  confirm('Are you sure you want to oper <?php echo $user->nick; ?>?')" name="<?php echo base64_encode($user->nick); ?>" value="Oper"></input>
                                    </form>
                                </td>
                    <?php
                            }
    ...
    <table class='dalektablethin'>
        ...
        ?>
            <tr>
                ...
                <td>
                    <form method="post">
                        <input type="submit" onclick="return  confirm('Are you sure you want to rehash <?php echo $serv->servername; ?>?')" name="<?php echo base64_encode($serv->servername); ?>" value="Rehash"></input>
                    </form>
                </td>
            </tr>
        <?php
    ...
    <table class='dalektablethin'>
        ...
        ?>
            <tr>
                ...
                <td>
                    <form method="post"><input type="submit" onclick="return  confirm('Are you sure? This cannot be reversed.')" name="<?php echo base64_encode($tkl->mask . MAGIC_SEP . $tkl->type); ?>" value="Remove"></input></form>
                </td>
                </td>
            </tr>
        <?php
